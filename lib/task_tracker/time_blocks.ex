defmodule TaskTracker.TimeBlocks do
  ...
  def block_time(time_block) do
    DateTime.diff(time_block.end_time, time_block.start_time, :minutes)
  end

  def total_time(task) do
    times = Enum.map(list_time_blocks(task.id), fn t -> block_time(t) end)
      |> Enum.sum
  end
end
